...
//! because we wish to also interoperate with hcnet-core which is based on the
...
//! which changed the rules, which we use in hcnet-core) as well as dalek's
//! `verify_strict` function. We test dalek in this file and have a
//! corresponding set of tests with the same test vectors in hcnet-core to
